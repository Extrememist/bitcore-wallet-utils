...
WalletUtils.PATHS = {
  BASE_ADDRESS_DERIVATION: {
    ...
    'BIP44': {
      livenet: "m/44'/0'/0'",
      testnet: "m/44'/1'/0'",
      dcrdlivenet: "m/44'/20'/0'",
      dcrdtestnet: "m/44'/11'/0'",
    },
  },
  REQUEST_KEY: "m/0/0",
...
WalletUtils.toSecret = function(walletId, walletPrivKey, network) {
  ...
  return _.padRight(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == 'dcrdtestnet' ? 'T' : 'L');
};
...
WalletUtils.fromSecret = function(secret) {
  ...
  try {
    var secretSplit = split(secret, [22, 75]);
    ...
    return {
      ...
      network: networkChar == 'T' ? 'dcrdtestnet' : 'dcrdlivenet',
    };
  } catch (ex) {
...
WalletUtils.getBaseAddressDerivationPath = function(derivationStrategy, network, account) {
  ...
  $.checkArgument(_.contains(['dcrdlivenet', 'dcrdtestnet'], network));
  $.shouldBeNumber(account);
  return "m/44'/" + (network == 'dcrdlivenet' ? "20'" : "11'") + "/" + account + "'";
};
...
WalletUtils.getNetworkFromExtendedKey = function(xKey) {
  ...
  return xKey.charAt(0) == 't' ? 'dcrdtestnet' : 'dcrdlivenet';
};
...
WalletUtils.formatAmount = function(atoms, unit, opts) {
